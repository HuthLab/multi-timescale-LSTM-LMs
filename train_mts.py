...
parser.add_argument('--baseline', action='store_true',default=False,
                    help='arg to train a standard LM instead of a MTS.')
...
def model_copy_save(fn):
    with open(fn, 'wb') as f:
        torch.save([model_copy, criterion], f)

def model_load(fn):
    global model
    with open(fn, 'rb') as f:
        model, criterion_m = torch.load(f)
    return 
...
#two copies of MTS LM model 
model_copy = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.dropouth, args.dropouti, args.dropoute, args.wdrop, args.tied)
...
#same as above function, it evaluates model_copy LM 
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            print("'"*89)
            #copying parameters from model_copy to model 
            l1 = dict(model.named_parameters())
            l2 = dict(model_copy.named_parameters())
            #note that we made changes in the for loop to make it compatible with Pytorch 1.7.0, however, our code is originally developed for Pytorch 1.0.0    
            for prm_name in l1.keys():
                try: 
                    l2[prm_name].data = copy.deepcopy(optimizer.state[l1[prm_name]]['ax'])
                except:
                    l2[prm_name].data = copy.deepcopy(l1[prm_name].data);
...
if True: #model ppx on test data  
